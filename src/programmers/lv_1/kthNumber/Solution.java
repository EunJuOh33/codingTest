package programmers.lv_1.kthNumber;
...
public class Solution {
    ...
    public int[] solution(int[] array, int[][] commands) {
        ...
        // 처음에 int[] answer = {}; 라고 잘못 설정하여 애를 먹었다. 배열의 길이를 정해줘야 한다는 점 잊지 말자!!!
        
        // 우리는 commands[?][0], commands[?][1] i와 k가 인덱스0, 인덱스1번째에 있다는 것을 알고 있다.
        // 하지만 commands = { {i,j,k}, {i,j,k}, {i,j,k} ... } 첫번째 인덱스는 50개 이하이므로 알 수 없음
        // answer 배열의 길이는 commands배열의 ([] [])첫번째 배열길이?와 같다.
        
        for(int i = 0; i < commands.length; i++){
        	int temp[] = Arrays.copyOfRange(array, commands[i][0] -1, commands[i][1]);	// { 5, 2, 6, 3}
        	// arry = { 1, 2, 3, 4, 5} -> (array, 2, 4) -> {3, 4} 첫번째 요소는 포함하지만 마지막 요소는 포함하지 않는다.
	        Arrays.sort(temp);	// 정렬하라고 했으니 꼭 정렬한다. -> { 2, 3, 5, 6 }
	        
	        answer[i] = temp[commands[i][2] -1];	// temp의 k번째 수를 answer[i]에 담는다.
        }
    ...
    public int[] solutionTest(int[] array, int[][] commands) {
        int[] answer = new int[commands.length];
    ...
    public static void main(String[] args) {
    	...
    	int[][] commands = { 
    							...
    							{4, 4, 1}, 
    							{1, 7, 3} 
    						};	// i와 j의 인덱스 자리는 commands[?][0], commands[?][1] 로 일정하다.
